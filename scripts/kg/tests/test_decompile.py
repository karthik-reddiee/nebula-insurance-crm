...
def _fixed_kg(tmp_path: Path) -> Path:
    """Copy the real KG into a temp dir and apply the one known source-drift fix (the 6 mis-filed
    `capability:document-*` records move glossary_terms → capabilities). Returns the temp kg dir."""
    kg = tmp_path / "kg"
    kg.mkdir()
    for f in ("canonical-nodes.yaml", "feature-mappings.yaml", "code-index.yaml", "solution-ontology.yaml"):
        shutil.copy(KG_DIR / f, kg / f)
    d = yaml.safe_load((kg / "canonical-nodes.yaml").read_text())
    move = [r for r in d["glossary_terms"] if r["id"].startswith("capability:")]
    d["glossary_terms"] = [r for r in d["glossary_terms"] if not r["id"].startswith("capability:")]
    d["capabilities"].extend(move)
    (kg / "canonical-nodes.yaml").write_text(kg_common.canonical_dump(d), encoding="utf-8")
    return kg


# ── the headline gate: compile(decompile(graph)) == graph, on the REAL graph ──
def test_real_graph_roundtrip_byte_identical(tmp_path):
    kg = _fixed_kg(tmp_path)
