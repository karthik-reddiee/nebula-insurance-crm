...
> **Status:** schema + ownership contract (F0006-S0004 / PRD row B1). The compiler (S0005), the
> migration that populates this tree (S0006), and tracker generation (S0007) land in later stories.
> Until the S0006 cutover, `planning-mds/knowledge-graph/*.yaml` remains the authoritative graph and
> this directory holds only the schema, the ownership contract, and validator fixtures.
...
**Not shards:** `story:` records stay projected from story files (STORY-INDEX is already generated),
and `coverage-report.yaml` stays a `validate.py --write-coverage-report` regeneration. Neither is
authored here.
...
Ownership is also encoded as write-scopes in `nebula-agents/agents/agent-map.yaml` (architect →
...
`retired`. The tracker-display mapping (e.g. `archived-done` → REGISTRY "Archived" table + ROADMAP
...
| ROADMAP · Abandoned | Feature←link(`id`,`name`,`path`), Superseded By←`superseded_by`, Rationale←`reason` |
| ROADMAP · Completed | Feature←link(`id`,`name`,`path`), Phase←`phase`, Completion State←`completion_state` |
